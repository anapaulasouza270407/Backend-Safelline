...
const matchingServices = require('./matching.services');
...
class webSocketService {
    ...
    initialize(io) {
        ...
        io.on('connection', (socket) => {
            ...
            socket.on('find-match', async (data) => {
                ...
                */
               const result = await matchingServices.joinQueue(socket.userId, socket.id, category);
            ...
            socket.on('cancel-match', () => {
                if (socket.userId) {
                    matchingServices.leaveQueue(socket.userID);
                    socket.emit('match-cancelled', {message: 'Matchmaking cancelled'});
                }});

            socket.on('Join-room', (data) =>{
                const room = matchingServices.getRoom(data.roomId);
        ...
        setInterval(() => {matchingService.cleanupInactiveRooms();}, 5 * 60 * 1000); /// Intervalo de 5 minutos em milessegundos
    }
    handleLeaveRoom(socket,roomId = null, isDisconnect = false) {
        ...
        if(targetRoom) {
            ...
            const roomData = matchingServices.leaveRoom(targetRoom, socket.userId);
            if(roomData) {
                ...
                if (roomData.partnerSocketID) {
                    const partnerSocket = this.io.sockets.get(roomData.partnerSocketID);
                    if (partnerSocket) {
...
//Exporta uma instancia unica (singleton) do webSocketService para ser usada e, toda a aplicação
